...
describe('Pattern Eval — php-conduit-publisher', () => {
    let chunks: CodeChunk[];
    let manifest: ReturnType<typeof loadFixtureManifest>;

    beforeAll(() => {
        console.log(`[Pattern Eval] php-conduit-publisher | Mode: ${EVAL_LLM_MODE}`);
    ...
    it('should load fixture code and manifest', () => {
        ...
        expect(manifest.fixture).toBe('php-conduit-publisher');
    });

    it('OrderService — should detect I/O via taint propagation', async () => {
        const chunk = chunks.find(c => c.name === 'OrderService');
        expect(chunk, 'Fixture must contain OrderService.php').toBeDefined();
